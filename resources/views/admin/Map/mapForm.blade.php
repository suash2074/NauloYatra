        ...
        <div class="col">
            <div class="card shadow">
                ...
                <div class="container mb-4">
                    ...
                    @endif
                    @if (isset($map_data))
                        <form action="{{ route('map.update', @$map_data->id) }}" method="post" class="form"
                            enctype="multipart/form-data">
                            @method('put')
                            @csrf
                        @else
                            <form action="{{ route('map.store') }}" method="post" class="form"
                                enctype="multipart/form-data">
                                @csrf
                    @endif
                    ...
                    <div class="row">
                        <div class="form-group col-6">
                            <label for="route_name" class="form-control-label">Route Name<span
                                    class="text-danger">*</span></label>
                            <input class="form-control" type="text"
                                placeholder="eg: Pokhara to KathmanduFormat longitude, latitude: 85.3247, 27.6588" name="route_name"
                                id="route_name" value="{{ @$map_data->route_name }}" required>
                            @error('route_name')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="form-group col-12">
                            <div class="form-group">
                                <label class="form-control-label" for="exampleFormControlTextarea1">Path Coordinates
